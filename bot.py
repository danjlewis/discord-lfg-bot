                            ...
                            if db["guilds"][str(channel.guild.id)]["dmnotify"]:
                                for client_user in users:
                                    try:
                                        ...
                                        if user.dm_channel == None:
                                            await user.create_dm()
                                        await user.dm_channel.send(f"{channel.guild.get_member(request['author']).display_name}'s {request['game']} group in {channel.guild.name} has enough players!")
                                    except (discord.Forbidden, AttributeError, discord.NotFound):
                                        pass
                            else:
                                try:
                                    await channel.send(f"{channel.guild.get_member(request['author']).display_name}'s {request['game']} group has enough players! " + \
